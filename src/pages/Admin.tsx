...
const Admin = () => {
  ...
  const handleAdminLogin = useCallback(() => {
    ...
    const adminPass = import.meta.env.VITE_ADMIN_PASSWORD || "Walied 2004";
    setTimeout(() => {
  ...
  const fetchProfessors = useCallback(async () => {
    try {
      ...
      const processedData = (data || []).map((prof: any) => ({
        ...prof,
        permissions: {
          attendance: prof.permissions?.attendance ?? true,
          grades: prof.permissions?.grades ?? true,
          meetings: prof.permissions?.meetings ?? true,
        },
      }));
    ...
    return (
      <div className="min-h-screen bg-gray-900 flex items-center justify-center p-4 relative overflow-hidden">
        ...
        <div className="fixed inset-0 z-0">
          ...
          >
            <source src={`${import.meta.env.BASE_URL}M.mp4`} type="video/mp4" />
          </video>
        </div>
    ...
    <div className="min-h-screen bg-gray-900 text-gray-100 relative overflow-hidden">
                    ...
                    <TableRow key={professor.id}>
                      ...
                      <TableCell>
                        <div className="flex flex-wrap gap-1">
                          {Object.entries(professor.permissions)
                            .filter(([key]) => permissionLabels[key as keyof typeof permissionLabels])
                            .map(
                            ([key, value]) => (
                              <Badge
                                key={key}
      ...
      >
        <DialogContent className="sm:max-w-md">
          ...
          {selectedProfessor && (
            <div className="space-y-4">
              {Object.entries(selectedProfessor.permissions)
                .filter(([key]) => permissionLabels[key as keyof typeof permissionLabels])
                .map(
                ([key, value]) => {
                  const Icon =
