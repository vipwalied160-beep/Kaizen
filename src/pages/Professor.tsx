    ...
    if (professorData) {
      ...
      if (codeData) {
        ...
        if (codeData.permissions) {
          const rawPerms = codeData.permissions as any;
          setPermissions({
            attendance: !!rawPerms.attendance,
            grades: !!rawPerms.grades,
            meetings: !!rawPerms.meetings,
          });
        }
      }
  ...
  const handleOpenAttendance = useCallback(async () => {
    ...
    if (!navigator.geolocation) {
      toast.error(language === "ar" 
        ? "⚠️ متصفحك لا يدعم تحديد الموقع أو يتطلب اتصال HTTPS" 
        : "⚠️ Browser doesn't support geolocation or requires HTTPS");
      return;
    }
